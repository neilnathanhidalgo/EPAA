...
import { getAsistenteById } from "../services/asistenteService";
...
const CuiLogin = ({ navigation }) => {
  ...
  useEffect(() => {
    getAsistenteById(1).then((data) => {
      if (data) {
        setAsistente(data);
        console.log(data);
      } else {
        setErrorMessage("No se pudo cargar los datos del asistente.");
      }
    });
    if (pinCode.length === 4) {
      const enteredPin = pinCode.join("");
      const currentPin = asistente.pin;
  ...
  return (
    <View style={styles.container}>
      ...
      <Text
        style={[styles.BText, { fontFamily: "Oswald", fontSize: width * 0.05 }]}
      >
        BIENVENIDO/A
      </Text>
      ...
      <Text
        style={[
          styles.NombreText,
          { fontFamily: "Oswald", fontSize: width * 0.09 },
        ]}
      >
      ...
      <Text
        style={[
          styles.cText,
          { fontFamily: "OldRegular", fontSize: width * 0.05 },
        ]}
      >
        Ingresa tu clave
      </Text>
        ...
        {[...Array(4).keys()].map((index) => (
          <View
            ...
            style={[
              styles.pinDot,
              {
                backgroundColor: pinCode.length > index ? "#006B78" : "#EFEFEF",
              },
            ]}
          />
        ))}
