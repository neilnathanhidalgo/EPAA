  ...
  useEffect(() => {
    if (isFocused) {
      intervalRef.current = setInterval(() => {
        ...
        getAlertaByType("Caída")
          .then((alertas) => {
            if (alertas.length > 0) {
              navigation.navigate("AlertaCaidaCui");
              clearInterval(intervalRef.current);
            }
          })
          .catch((error) => {
            console.error("Error al obtener alertas de Caída:", error);
          });
        getAlertaByType("Ayuda")
          .then((alertas) => {
            if (alertas.length > 0) {
              navigation.navigate("AlertaAyudaCui");
              clearInterval(intervalRef.current);
            }
          })
          .catch((error) => {
            console.error("Error al obtener alertas de Ayuda:", error);
          });
        getAlertaByType("Conexión")
          .then((alertas) => {
            if (alertas.length > 0) {
              const total = alertas.length;
              const atendidas = alertas.filter(
                (alerta) => alerta.atendida === "S"
              ).length;
              if (total === atendidas) {
                navigation.navigate("AlertaRangoCui");
                clearInterval(intervalRef.current);
              } else {
                alertas
                  .filter((alerta) => alerta.atendida === "N")
                  .forEach((alerta) => {
                    updateAlerta(alerta.id_alerta, {
                      ...alerta,
                      atendida: "S",
                    });
                    console.log("Alerta de Conexión atendida:", alerta);
                  });
              }
            } else {
              console.log("No hay alertas de Conexión");
            }
          })
          .catch((error) => {
            console.error("Error al obtener alertas de Conexión:", error);
          });
      }, 3000);

      intervalConexionRef.current = setInterval(() => {}, 5000);
    }
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.cuidadoresContainer}>
        <Text
          style={[styles.c, { fontFamily: "Oswald", fontSize: width * 0.105 }]}
        >
          USUARIOS ASISTIDOS
        </Text>
      </View>
